...
export class TaskScheduler implements ITaskScheduler {
  ...
  async cancelTask(taskId: string): Promise<void> {
    ...
    for (const queue of this.queues.values()) {
      queue.tasks = queue.tasks.filter((task: Task) => task.id !== taskId);
    }
  ...
  async updateTaskPriority(taskId: string, priority: TaskPriority): Promise<void> {
    ...
    for (const queue of this.queues.values()) {
      const taskIndex = queue.tasks.findIndex((task: Task) => task.id === taskId);
  ...
  private evaluateFilter(task: Task, filter: TaskFilter): boolean {
    ...
    return this.compareValues(value, filter.value as string | number | string[] | undefined, filter.operator);
  }
  ...
  private sortQueue(queue: TaskQueue): void {
    switch (queue.processingStrategy) {
      case "PRIORITY":
        queue.tasks.sort((a: Task, b: Task) => b.priority - a.priority);
        break;
      case "LIFO":
        queue.tasks.sort((a: Task, b: Task) => b.createdAt.getTime() - a.createdAt.getTime());
        break;
      case "FIFO":
      default:
        queue.tasks.sort((a: Task, b: Task) => a.createdAt.getTime() - b.createdAt.getTime());
        break;
    }
  }
