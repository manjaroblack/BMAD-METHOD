...
class Installer {
		...
		try {
			...
			for (const configFile of agentConfigs) {
				...
				if (agentName === "bmad-master") {
					agentConfig = this.buildBmadMasterConfig(agentConfig, allAgentConfigs);
				}
	...
	async performFreshInstall(config, installDir, spinner, options = {}) {
		...
		if (config.installType === "full") {
			...
			const generatedAgentFiles = await this.generateAgentsFromConfigs(bmadCoreDestDir, spinner);
		...
		if (ides.length > 0) {
			for (const ide of ides) {
				...
				const preConfiguredSettings =
					ide === "github-copilot" ? config.githubCopilotConfig : null;
