...
class Installer {
  ...
  async performFreshInstall(config, installDir, spinner, options = {}) {
    ...
    if (config.installType === 'full') {
      ...
      const destAgentPath = path.join(installDir, '.bmad-core', 'agents', `${config.agent}.md`)
  ...
  async installWebBundles(webBundlesDirectory, config, spinner) {
    try {
      // Find the bundles directory in the BMad installation
      const bundlesDir = configLoader.getBundlesPath()

      if (!(await fileManager.pathExists(bundlesDir))) {
      ...
      if (webBundleType === 'all') {
        // Copy the entire bundles directory structure
        await fileManager.copyDirectory(bundlesDir, webBundlesDirectory)
        ...
        if (
          ...
          const agentsSource = path.join(bundlesDir, 'agents')
        ...
        if (webBundleType === 'teams' || webBundleType === 'custom') {
          if (config.selectedWebBundleTeams && config.selectedWebBundleTeams.length > 0) {
            const teamsSource = path.join(bundlesDir, 'teams')
        ...
        const expansionSource = path.join(bundlesDir, 'expansion-packs')
