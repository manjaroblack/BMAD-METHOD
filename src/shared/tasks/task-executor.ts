...
export class TaskExecutor implements ITaskExecutor {
  ...
  getActiveExecutions(): TaskExecution[] {
    return Array.from(this.executions.values()).filter(
      (execution) =>
        execution.status === TaskStatus.RUNNING || execution.status === TaskStatus.PAUSED,
    );
  }
  ...
  addCheckpoint(taskId: string, checkpoint: TaskCheckpoint): void {
    ...
    if (execution) {
      ...
      this.logMessage(
        taskId,
        "info",
        `Checkpoint: ${checkpoint.description} (${checkpoint.progress}%)`,
      );
    }
  }
...
export class DefaultTaskExecutorStrategy implements TaskExecutorStrategy {
  async execute(
    ...
    return {
      ...
      metrics: {
        executionTime: Date.now() - _execution.startTime.getTime(),
      },
    };
  }
