...
export class WorkflowEngine implements IWorkflowEngine {
  ...
  async executeWorkflow(
    ...
    const execution: WorkflowExecution = {
      ...
      startTime: new Date(),
      context: { ...context },
      stepExecutions: definition.steps.map((step: WorkflowStep) => ({
        stepId: step.id,
  ...
  getActiveExecutions(): WorkflowExecution[] {
    return Array.from(this.executions.values()).filter(
      (execution) =>
        execution.status === WorkflowStatus.RUNNING || execution.status === WorkflowStatus.PAUSED,
    );
  }
  ...
  getMetrics(executionId: string): WorkflowMetrics | undefined {
    ...
    const completedSteps =
      execution.stepExecutions.filter((s: StepExecution) => s.status === StepStatus.COMPLETED).length;
    const failedSteps =
      execution.stepExecutions.filter((s: StepExecution) => s.status === StepStatus.FAILED).length;
    const skippedSteps =
      execution.stepExecutions.filter((s: StepExecution) => s.status === StepStatus.SKIPPED).length;
    ...
    const stepTimes = execution.stepExecutions
      .filter((s: StepExecution) => s.startTime && s.endTime)
      .map((s: StepExecution) => s.endTime!.getTime() - s.startTime!.getTime());
  ...
  private async executeSteps(
    ...
    const pendingSteps = new Set(definition.steps.map((s: WorkflowStep) => s.id));
  ...
  private async executeStep(
    ...
    const stepExecution = execution.stepExecutions.find((s: StepExecution) => s.stepId === step.id);
    if (!stepExecution) {
