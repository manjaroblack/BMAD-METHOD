...
export class ExtensionRegistry implements IExtensionRegistry {
  ...
  private getDependencyStats() {
    ...
    return {
      ...
      mostDependentExtension: extensions.length > 0 ? extensions.reduce((max, ext) => 
  ...
  private getHealthStats() {
    const extensions = this.getAll();
    const healthy = extensions.filter(ext => ext.status === ExtensionStatus.ACTIVE && !ext.errors?.length);
  ...
  private getTimelineStats() {
    ...
    const oldest = extensions.reduce((min, ext) => 
      ext.loadedAt! < min.loadedAt! ? ext : min
    );
    
    const newest = extensions.reduce((max, ext) => 
      ext.loadedAt! > max.loadedAt! ? ext : max
    );
    
    const now = Date.now();
    const averageAge = extensions.reduce((sum, ext) => 
      sum + (now - ext.loadedAt!.getTime()), 0
    ) / extensions.length;
