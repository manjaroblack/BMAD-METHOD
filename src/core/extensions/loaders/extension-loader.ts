...
} from '../types/extension.types.ts';

export interface IExtensionLoader {
  loadExtensionConfig(extensionPath: string): Promise<ExtensionConfig>;
  loadExtensionResources(extensionPath: string, config: ExtensionConfig): Promise<ExtensionResources>;
...
export class ExtensionLoader implements IExtensionLoader {
  ...
  async loadExtensionConfig(extensionPath: string): Promise<ExtensionConfig> {
    try {
      ...
      return {
        id: config.id || this.generateExtensionId(extensionPath),
        name: config.name || 'Unknown Extension',
      ...
      throw new Error(`Failed to load extension config from ${extensionPath}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  async loadExtensionResources(extensionPath: string, _config: ExtensionConfig): Promise<ExtensionResources> {
  ...
  private async loadChecklistResources(checklistsPath: string): Promise<ExtensionChecklistResource[]> {
