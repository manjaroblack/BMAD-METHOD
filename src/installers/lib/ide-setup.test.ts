...
describe("IdeSetup", () => {
  ...
  let ensureDirStub: Stub<IFileManager, [string], Promise<void>>;
  let writeTextFileStub: Stub<IFileManager, [string, string], Promise<void>>;
  let readTextFileStub: Stub<IFileManager, [string], Promise<string>>;
  let existsStub: Stub<IFileManager, [string], Promise<boolean>>;
  let readDirStub: Stub<IFileManager, [string], AsyncIterable<Deno.DirEntry>>;
  ...
  beforeEach(() => {
    ...
    readTextFileStub = stub(
      mockFileManager,
      "readTextFile",
      (_path: string) => Promise.resolve("test content"),
    );
    existsStub = stub(
      mockFileManager,
      "exists",
  ...
  it("should handle multiple IDE configurations", async () => {
    const spinner = { text: "" } as ISpinner;
    await ideSetup.setupIdeConfigurations(
      "/test/path",
      ["cursor", "cline"],
      spinner,
    );
