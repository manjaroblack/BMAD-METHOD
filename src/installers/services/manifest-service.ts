...
export class ManifestService implements IManifestService {
  ...
  async createInstallManifest(installDir: string): Promise<void> {
    ...
    try {
      ...
      this.logger?.error('Failed to create installation manifest', error as Error, {
        installDir,
        manifestPath
      });
  ...
  async readInstallManifest(installDir: string): Promise<Record<string, unknown> | null> {
    const manifestPath = this.getManifestPath(installDir);
    ...
    try {
      ...
      this.logger?.error('Failed to read installation manifest', error as Error, {
        installDir,
        manifestPath
      });
  ...
  async updateInstallManifest(
    ...
    try {
      ...
      this.logger?.error('Failed to update installation manifest', error as Error, {
        installDir,
        updates
      });
  ...
  async validateManifest(installDir: string): Promise<boolean> {
    ...
    try {
      ...
      if (!manifest) {
        this.logger?.warn('Manifest validation failed: manifest does not exist', { installDir });
        return false;
      }
      ...
      if (isValid) {
        ...
        this.logger?.warn('Manifest validation failed: invalid structure', { installDir });
      }

      return isValid;

    } catch (error) {
      this.logger?.error('Failed to validate manifest', error as Error, { installDir });
      return false;
    }
  }

  async getManifestInfo(installDir: string): Promise<InstallationManifest | null> {
    const manifest = await this.readInstallManifest(installDir);
  ...
  private detectInstalledExpansionPacks(installDir: string): Record<string, unknown> {
  ...
  private async generateFileList(installDir: string): Promise<string[]> {
    ...
    try {
      ...
      this.logger?.error('Failed to generate file list', error as Error, { installDir });
      return [];
    }
  }

  private generateIntegrityChecksums(installDir: string): Record<string, unknown> {
    ...
    try {
      ...
      this.logger?.error('Failed to generate integrity checksums', error as Error, { installDir });
      return {};
    }
  }
  ...
  private isValidManifestStructure(manifest: Record<string, unknown>): boolean {
    ...
    return requiredFields.every(field => 
      field in manifest && manifest[field] !== null && manifest[field] !== undefined
    );
  }
}
