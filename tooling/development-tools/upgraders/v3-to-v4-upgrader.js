...
class V3ToV4Upgrader {
  ...
  showPreflightCheck(analysis, options) {
  ...
  async installV4Structure(projectPath) {
    ...
    try {
      // Get the source src/core directory
    const sourcePath = path.join(__dirname, "..", "..", "src", "core");
  ...
  async setupIDE(projectPath, selectedIdes) {
    ...
    const ideSetup = require("../installers/lib/ide-setup.js");
  ...
  async createInstallManifest(projectPath) {
    const fileManager = require("../installers/lib/file-manager.js");
