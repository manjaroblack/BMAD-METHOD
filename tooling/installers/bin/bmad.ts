...
let installer: any; // Complex dynamic import type, keeping as any for compatibility
...
async function initializeInstaller(): Promise<void> {
  try {
    ...
    const installerModule = await import("../lib/installer.ts");
    installer = installerModule.default;
  } catch (e) {
...
cli.command("list:expansions")
  ...
  .action(async () => {
    try {
      ...
      expansions.forEach((pack: { id: string; shortTitle: string; version: string }) => {
        console.log(
