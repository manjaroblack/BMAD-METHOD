...
import { join, exists, ensureDir } from "deps";
import { InstallerOrchestrator } from '../../core/installer-orchestrator.ts';
import { InstallationDetector } from '../../core/installation-detector.ts';
// import { FreshInstallHandler } from '../../handlers/fresh-install-handler.ts';
// import { UpdateHandler } from '../../handlers/update-handler.ts';
// import { RepairHandler } from '../../handlers/repair-handler.ts';
// import { UnknownInstallHandler } from '../../handlers/unknown-install-handler.ts';
import { ManifestService } from '../../services/manifest-service.ts';
import { ExpansionPackService } from '../../services/expansion-pack-service.ts';
import { IntegrityChecker } from '../../services/integrity-checker.ts';
// import { CoreInstaller } from '../../services/core-installer.ts';
import { logger } from '../../../../shared/services/core/logger.service.ts';
import { performanceMonitor } from '../../../../shared/services/core/performance.service.ts';
import { spinner } from '../../../../shared/services/core/spinner.service.ts';
import { fileSystemService } from '../../../../shared/services/utils/file-system.service.ts';
import type { InstallConfig } from '../../../../shared/types/installation.types.ts';
...
class TestUtils {
  ...
  static async createMockSourceFiles(): Promise<void> {
    ...
    const coreDir = join(TEST_CONFIG.sourceDir, 'src/core');
    await ensureDir(join(coreDir, 'agents'));
    await ensureDir(join(coreDir, 'workflows'));
    await ensureDir(join(coreDir, 'checklists'));
    
    await Deno.writeTextFile(join(coreDir, 'agents/test-agent.ts'), '// Mock agent file');
    await Deno.writeTextFile(join(coreDir, 'workflows/test-workflow.ts'), '// Mock workflow file');
    await Deno.writeTextFile(join(coreDir, 'checklists/test-checklist.ts'), '// Mock checklist file');

    // Create mock expansion pack
    const expansionDir = join(TEST_CONFIG.sourceDir, 'extensions/test-pack');
    await ensureDir(expansionDir);
    await Deno.writeTextFile(join(expansionDir, 'manifest.json'), JSON.stringify({
      name: 'test-pack',
      version: '1.0.0',
    ...
    await Deno.writeTextFile(join(toolingDir, 'test-tool.ts'), '// Mock tool file');
  }
  ...
  static async createCorruptedInstallation(): Promise<void> {
    ...
    const manifest = {
      ...
      coreVersion: '1.0.0',
      files: [
        'src/core/agents/test-agent.ts',
        'src/core/workflows/test-workflow.ts',
    ...
    const coreDir = join(TEST_CONFIG.installDir, 'src/core');
    await ensureDir(join(coreDir, 'agents'));
    await Deno.writeTextFile(join(coreDir, 'agents/test-agent.ts'), '// Existing agent file');
...
Deno.test("Installer Integration Tests", async (t) => {
  ...
  await t.step("Fresh Installation Flow", async () => {
    ...
    const agentFile = join(TEST_CONFIG.installDir, 'src/core/agents/test-agent.ts');
    assertExists(await exists(agentFile));
  });

  await t.step("Update Installation Flow", async () => {
    // Modify source files to simulate updates
    const sourceAgent = join(TEST_CONFIG.sourceDir, 'src/core/agents/test-agent.ts');
    ...
    const updatedContent = await Deno.readTextFile(
      join(TEST_CONFIG.installDir, 'src/core/agents/test-agent.ts')
    );
  ...
  await t.step("Repair Installation Flow", async () => {
    ...
    const workflowFile = join(TEST_CONFIG.installDir, 'src/core/workflows/test-workflow.ts');
  ...
  await t.step("Expansion Pack Installation", async () => {
    ...
    const packManifest = join(TEST_CONFIG.installDir, 'extensions/test-pack/manifest.json');
  ...
  await t.step("Performance Benchmarks", async () => {
    ...
    assertEquals(totalTime < 5000, true, `Installation took ${totalTime}ms, expected < 5000ms`);
  });
  ...
  await t.step("Memory Usage Tracking", async () => {
    ...
    assertEquals(memMB < 100, true, `Memory usage ${memMB}MB exceeds 100MB threshold`);
  });
  ...
  await t.step("Concurrent Installation Handling", async () => {
    ...
    assertEquals(successCount >= 1, true, 'At least one concurrent installation should succeed');
  });
...
export class PerformanceBenchmark {
  static async benchmarkInstallationTypes(): Promise<void> {
    ...
    const configs = [
      { name: 'Minimal Core', config: { selectedPacks: ['.bmad-core'], expansionOnly: false } },
      { name: 'Full Installation', config: { selectedPacks: ['.bmad-core'], full: true } },
      { name: 'Expansion Only', config: { selectedPacks: ['test-pack'], expansionOnly: true } }
    ];
