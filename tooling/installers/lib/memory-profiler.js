...
class MemoryProfiler {
  ...
  getRecommendations(memoryGrowth) {
    ...
    if (largeGrowths.length > 0) {
      recommendations.push({
        type: "warning",
        message:
          `Large memory growth detected in ${largeGrowths.length} operations`,
        details: largeGrowths.map((g) =>
          `${g.from} → ${g.to}: ${g.heapGrowth}`
        ),
      });
    }
  ...
  checkContinuousGrowth() {
    ...
    for (let i = 1; i < this.checkpoints.length; i++) {
      if (
        this.checkpoints[i].raw.heapUsed > this.checkpoints[i - 1].raw.heapUsed
      ) {
        increasingCount++;
      }
    }
