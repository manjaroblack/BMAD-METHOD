...
import {
  ...
  InvalidValueError
} from '../../../shared/errors/validation.errors.ts';
...
class Installer {
  ...
  async getInstallationStatus(directory: string): Promise<Record<string, unknown>> {
    try {
      ...
      logger.error('Failed to get installation status', error as Error, { directory });
  ...
  flatten(options: Record<string, unknown>): void {
    try {
      ...
      console.log(yellow('⚠️  Flatten operation not yet implemented in modular architecture'));
  ...
  private validateConfig(config: InstallConfig): void {
    ...
    if (config.selectedPacks && !Array.isArray(config.selectedPacks)) {
      throw new InvalidValueError('selectedPacks', config.selectedPacks, 'array');
    }
  ...
  private handleInstallationError(error: Error, config: InstallConfig): void {
    ...
    if (error instanceof InstallationError) {
      ...
      console.log(red(`❌ Installation Error [${error.code}]: ${error.message}`));
      return;
    }

    // Handle specific error patterns
    if (error.message.includes('ENOENT') || error.message.includes('not found')) {
      const dirError = new DirectoryNotFoundError(config.directory || 'unknown');
    ...
    if (error.message.includes('EACCES') || error.message.includes('permission')) {
      const permError = new InsufficientPermissionsError(config.directory || 'unknown');
  ...
  private handleUpdateError(error: Error, options: Record<string, unknown>): void {
    if (error instanceof InstallationError) {
      logger.error('Update error occurred', error, { 
        options, 
        errorCode: error.code 
      });
      console.log(red(`❌ Update Error [${error.code}]: ${error.message}`));
      return;
    }
