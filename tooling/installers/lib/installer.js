...
import InstallerValidator from "./installer-validator.js";
import { Logger, ValidationError } from "../../lib/error-handler.js";
...
class Installer {
  ...
  async generateAgentsFromConfigs(bmadCoreDestDir, spinner) {
    try {
      const agentConfigsDir = path.join(
        resourceLocator.getBmadCorePath(),
        "agent-configs",
      );
      const agentsDir = path.join(bmadCoreDestDir, "agents");
      ...
      // Load base template
      const baseTemplatePath = path.join(templatesDir, "agent-base-tmpl.yaml");
      const baseTemplateContent = await fs.readFile(baseTemplatePath, "utf8");
      const baseTemplate = yaml.load(baseTemplateContent);

      // Get all agent config files
      const configFiles = await fs.readdir(agentConfigsDir);
      const agentConfigs = configFiles.filter((file) =>
      ...
      for (const configFile of agentConfigs) {
        ...
        const configPath = path.join(agentConfigsDir, configFile);
        const agentConfigContent = await fs.readFile(configPath, "utf8");
        allAgentConfigs[agentName] = yaml.load(agentConfigContent);
      }
      ...
      for (const configFile of agentConfigs) {
        ...
        let agentConfig = allAgentConfigs[agentName];
      ...
      console.error(
        chalk.red("Failed to generate agents from configs:"),
        error.message,
      );
      return [];
    }
  ...
  buildBmadMasterConfig(bmadMasterConfig, allAgentConfigs) {
    // Start with the core bmad-master config
    const masterConfig = JSON.parse(JSON.stringify(bmadMasterConfig));

    // Initialize aggregated dependencies
    ...
    // Add core dependencies from bmad-master config
    if (masterConfig.core_dependencies) {
      Object.keys(aggregatedDependencies).forEach((depType) => {
        if (masterConfig.core_dependencies[depType]) {
          masterConfig.core_dependencies[depType].forEach((item) => {
            aggregatedDependencies[depType].add(item);
          });
        }
      });
    }

    // Aggregate dependencies from all other agents except bmad-orchestrator and dev
    const excludedAgents = ["bmad-orchestrator", "dev", "bmad-master"];
    Object.keys(allAgentConfigs).forEach((agentName) => {
      if (!excludedAgents.includes(agentName)) {
        const agentConfig = allAgentConfigs[agentName];
        if (agentConfig.dependencies) {
          Object.keys(aggregatedDependencies).forEach((depType) => {
            if (agentConfig.dependencies[depType]) {
              agentConfig.dependencies[depType].forEach((item) => {
                aggregatedDependencies[depType].add(item);
              });
            }
          });
    ...
    // Convert Sets back to arrays and assign to dependencies
    masterConfig.dependencies = {};
    Object.keys(aggregatedDependencies).forEach((depType) => {
      const items = Array.from(aggregatedDependencies[depType]).sort();
      if (items.length > 0) {
        masterConfig.dependencies[depType] = items;
      }
    });

    // Remove core_dependencies as it's no longer needed
    delete masterConfig.core_dependencies;

    return masterConfig;
  }

  generateAgentFileContent(baseTemplate, agentConfig) {
    // Generate activation notice
    const activationNotice = baseTemplate.activation_notice;

    // Generate request resolution
    let requestResolution = baseTemplate.request_resolution_template;
    if (agentConfig.request_resolution_examples) {
      const examples = agentConfig.request_resolution_examples;
      requestResolution = requestResolution
        .replace(
          "{{example_request}}",
          examples.example_request || "example request",
        )
        .replace(
          "{{example_command}}",
          examples.example_command || "*example-command",
        )
        .replace(
          "{{example_request_2}}",
          examples.example_request_2 || "another request",
        )
        .replace(
          "{{example_task}}",
          examples.example_task || "example-task.md",
        );
    }

    // Generate activation instructions
    let activationInstructions = [];
    activationInstructions = activationInstructions.concat(
      baseTemplate.activation_instructions_base.core_steps,
    );
    activationInstructions = activationInstructions.concat(
      baseTemplate.activation_instructions_base.restrictions,
    );
    activationInstructions = activationInstructions.concat(
      baseTemplate.activation_instructions_base.workflow_rules,
    );
    activationInstructions = activationInstructions.concat(
      baseTemplate.activation_instructions_base.interaction_guidelines,
    );

    if (agentConfig.additional_activation_instructions) {
      activationInstructions = activationInstructions.concat(
        agentConfig.additional_activation_instructions,
      );
    }

    // Generate commands
    const commands = {};
    commands.help = baseTemplate.standard_commands.help.description;

    if (agentConfig.commands) {
      Object.assign(commands, agentConfig.commands);
    }

    if (agentConfig.include_standard_commands) {
      agentConfig.include_standard_commands.forEach((cmdName) => {
        if (baseTemplate.standard_commands[cmdName] && !commands[cmdName]) {
          commands[cmdName] =
            baseTemplate.standard_commands[cmdName].description;
        }
      });
    }

    const exitDesc = baseTemplate.standard_commands.exit.description.replace(
      "{{agent_title}}",
      agentConfig.agent.title,
    );
    commands.exit = exitDesc;

    // Build YAML content
    const yamlContent = {
      "IDE-FILE-RESOLUTION": baseTemplate.ide_file_resolution,
      "REQUEST-RESOLUTION": requestResolution,
      "activation-instructions": activationInstructions,
      agent: agentConfig.agent,
      persona: agentConfig.persona,
    };

    // Add agent-specific sections
    if (agentConfig.additional_sections) {
      Object.assign(yamlContent, agentConfig.additional_sections);
    }

    yamlContent.commands = commands;

    if (agentConfig.dependencies) {
      yamlContent.dependencies = agentConfig.dependencies;
    }

    // Generate final content
    let content = `# ${agentConfig.agent.id}\n\n`;
    content += activationNotice + "\n\n";
    content += "```yaml\n";
    content += yaml.dump(yamlContent, {
      indent: 2,
      lineWidth: 120,
      noRefs: true,
      sortKeys: false,
  ...
  async install(config) {
    ...
    this.monitor.startTimer('full_installation');

    try {
      // Validate prerequisites first
      const prerequisiteIssues = await this.validator.validatePrerequisites();
      
      const errors = prerequisiteIssues.filter(issue => issue.type === 'error');
      if (errors.length > 0) {
        spinner.fail(chalk.red('Prerequisites validation failed:'));
        errors.forEach(error => console.log(chalk.red(`  ✗ ${error.message}`)));
        return false;
      }
      
      const warnings = prerequisiteIssues.filter(issue => issue.type === 'warning');
      if (warnings.length > 0) {
        warnings.forEach(warning => console.log(chalk.yellow(`  ⚠ ${warning.message}`)));
      }

      spinner.text = "Analyzing installation directory...";

      try {
      // Store the original CWD where npx was executed
      const originalCwd = process.env.INIT_CWD || process.env.PWD ||
        process.cwd();
      ...
      switch (state.type) {
        ...
        this.monitor.endTimer('full_installation');
  ...
  async performFreshInstall(config, installDir, spinner, options = {}) {
    ...
    if (config.installType === "full") {
      ...
      // Generate agent files from configurations
      spinner.text = "Generating agent files from configurations...";
      const _generatedAgentFiles = await this.generateAgentsFromConfigs(
        bmadCoreDestDir,
        spinner,
      );
      ...
      for (const dep of dependencies) {
        ...
        if (dep.includes("*")) {
          ...
          const needsRootReplacement = dep.endsWith(".md") ||
            dep.endsWith(".yaml") ||
            dep.endsWith(".yml");
          let success = false;
    ...
    if (ides.length > 0) {
      for (const ide of ides) {
        ...
        const preConfiguredSettings = ide === "github-copilot"
  ...
  async handleExistingV4Installation(config, installDir, state, spinner) {
    ...
    switch (action) {
      ...
      case "expansions": {
        ...
        const _expansionFiles = await this.installExpansionPacks(
          installDir,
          selectedPacks,
          spinner,
          { ides: config.ides || [] },
  ...
  async handleV3Installation(config, installDir, spinner) {
    spinner.stop();
    ...
    switch (action) {
      case "upgrade": {
        ...
        const V3ToV4Upgrader = require("../../development-tools/upgraders/v3-to-v4-upgrader.js");
  ...
  async performRepair(installDir, manifest, integrity, spinner) {
  ...
  async listExpansionPacks() {
    ...
    console.log(
      chalk.dim(
        "Install with: npx bmad-method install --full --expansion-packs <id>\n",
      ),
    );
  }
  ...
  async getAvailableAgents() {
    return await resourceLocator.getAvailableAgents();
  }

  async getAvailableExpansionPacks() {
    return await resourceLocator.getExpansionPacks();
  }

  async getAvailableTeams() {
    return await configLoader.getAvailableTeams();
  }
    ...
    for (const packId of selectedPacks) {
      ...
      try {
        ...
        if (await fileManager.pathExists(existingManifestPath)) {
          ...
          const hasPackIntegrityIssues = packIntegrity.missing.length > 0 ||
            ...
            const { action } = await inquirer.prompt([
              {
                ...
                name: "action",
                message:
                  `${pack.name} v${pack.version} is already installed. What would you like to do?`,
                choices: choices,
              },
            ]);
            ...
            const { proceed } = await inquirer.prompt([
              {
                ...
                name: "proceed",
                message:
                  `Upgrade ${pack.name} from v${existingManifest.version} to v${pack.version}?`,
                default: true,
              },
            ]);
        ...
        for (const folder of foldersToSync) {
          ...
          if (await fileManager.pathExists(sourceFolder)) {
            ...
            for (const file of files) {
              ...
              const needsRootReplacement = file.endsWith(".md") ||
                file.endsWith(".yaml") ||
                file.endsWith(".yml");
              let success = false;
        ...
        for (const agentId of agents) {
          if (!existingAgents.has(agentId)) {
            ...
            if (await fileManager.pathExists(coreAgentPath)) {
              ...
              console.warn(
                chalk.yellow(
                  `  Warning: Core agent ${agentId} not found for team ${
                    path.basename(teamFile, ".yaml")
                  }`,
                ),
              );
            }
  ...
  async installWebBundles(webBundlesDirectory, config) {
    try {
  ...
  async copyCommonItems(installDir, targetSubdir) {
    const fs = require("fs").promises;
    const sourceBase = path.dirname(
      path.dirname(path.dirname(path.dirname(__filename))),
    ); // Go up to project root
  ...
  async repairExpansionPack(installDir, packId, pack, integrity, spinner) {
    ...
    try {
      const _expansionDotFolder = path.join(installDir, `.${packId}`);
  ...
  async cleanupLegacyYmlFiles(installDir) {
    const fs = require("fs").promises;

    try {
      // Find all .yml files in the installation directory
      const ymlFiles = await glob("**/*.yml", {
        cwd: installDir,
