...
class Installer {
  private validator: typeof installerValidator;
  private logger: Logger;
  private monitor: PerformanceMonitor;

  constructor() {
    this.validator = installerValidator;
  ...
  async setupIdeConfigurations(
    ...
    for (const ide of ides) {
      ...
      await ideSetup.configure(installDir, ide);
    }
  }
  ...
  async copyCommonItems(
    ...
    const commonPath = join(sourceBase, "common");
    ...
    for await (
      ...
      if (yamlContent) {
        try {
          ...
          console.warn(
            yellow(
              `Warning: Could not parse YAML in ${entry.path}: ${
                error instanceof Error ? error.message : String(error)
              }`,
            ),
          );
