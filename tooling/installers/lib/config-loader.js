...
class ConfigLoader {
          ...
          try {
            ...
            if (yamlContentText) {
              ...
              agents.push({
                ...
                file: `src/core/agents/${entry.name}`,
        ...
        if (entry.isDirectory() && !entry.name.startsWith(".")) {
          ...
          try {
            ...
            expansionPacks.push({
              ...
              description: config["short-title"] || config.description ||
                "No description available",
              fullDescription: config.description || config["short-title"] ||
                "No description available",
  ...
  getBmadCorePath() {
    // Get the path to src/core relative to the installer
    return path.join(__dirname, "..", "..", "..", "src", "core");
  }
    ...
    try {
      ...
      for (const resource of teamDeps.resources) {
        const filePath = `.bmad-core/${resource.type}/${resource.id}.${
          resource.type === "workflows" ? "yaml" : "md"
        }`;
        if (!depPaths.includes(filePath)) {
          depPaths.push(filePath);
        }
