...
class BaseIdeSetup {
  ...
  async createAgentRuleContent(agentId, agentPath, installDir, format = "mdc") {
    ...
    if (format === "mdc") {
      ...
      content +=
        `The complete agent definition is available in [${relativePath}](mdc:${relativePath}).\n\n`;
      content += "## Usage\n\n";
      content +=
        `When the user types \`@${agentId}\`, activate this ${agentTitle} persona and follow all instructions defined in the YAML configuration above.\n`;
