...
class IdeSetup extends BaseIdeSetup {
  ...
  async setupWindsurf(installDir, selectedAgent) {
    ...
    for (const agentId of agents) {
      ...
      if (agentPath) {
        ...
        mdContent +=
          `This rule is triggered when the user types \`@${agentId}\` and activates the ${await this
            .getAgentTitle(
              agentId,
              installDir,
            )} agent persona.\n\n`;
        ...
        const relativePath = path.relative(installDir, agentPath).replace(
          /\\/g,
          "/",
        );
        mdContent +=
        ...
        mdContent += "## Usage\n\n";
        mdContent +=
          `When the user types \`@${agentId}\`, activate this ${await this
            .getAgentTitle(
              agentId,
              installDir,
  ...
  async findAgentPath(agentId, installDir) {
    ...
    const expansionDirs = await glob(".*/agents", { cwd: installDir });
  ...
  async getAllAgentIds(installDir) {
    ...
    for (const expDir of expansionDirs) {
      ...
      const expAgentFiles = await glob("*.md", { cwd: fullExpDir });
      allAgentIds.push(
  ...
  async getCoreAgentIds(installDir) {
    ...
    // Check core agents in .bmad-core
  ...
  async getCoreTaskIds(installDir) {
    ...
    const tasksDir = path.join(installDir, ".bmad-core", "tasks");
    ...
    if (await fileManager.pathExists(commonTasksDir)) {
      const commonTaskFiles = await glob("*.md", { cwd: commonTasksDir });
      allTaskIds.push(
  ...
  async getAllTaskIds(installDir) {
    ...
    const expansionDirs = await glob(".*/tasks", { cwd: installDir });
    for (const expDir of expansionDirs) {
      const fullExpDir = path.join(installDir, expDir);
      const expTaskFiles = await glob("*.md", { cwd: fullExpDir });
      allTaskIds.push(
    ...
    if (await fileManager.pathExists(expansionPacksDir)) {
      const expPackDirs = await glob("*/tasks", { cwd: expansionPacksDir });
  ...
  async getInstalledExpansionPacks(installDir) {
    ...
    const dotExpansions = await glob(".bmad-*", { cwd: installDir });
    ...
    const expansionPacksDir = path.join(installDir, "expansion-packs");
    if (await fileManager.pathExists(expansionPacksDir)) {
      const packDirs = await glob("*", { cwd: expansionPacksDir });
    ...
    for (const agentId of agents) {
      ...
      if (agentPath) {
        ...
        if (yamlMatch) {
          ...
          newModesContent +=
            `   customInstructions: CRITICAL Read the full YAML from ${relativePath} start activation to alter your state of being follow startup section instructions stay in this being until told to exit this mode\n`;
  ...
  async setupCline(installDir, selectedAgent) {
    ...
    for (const agentId of agents) {
      ...
      if (agentPath) {
        ...
        let mdContent = `# ${await this.getAgentTitle(
          agentId,
          installDir,
        )} Agent\n\n`;
        mdContent += `This rule defines the ${await this.getAgentTitle(
          agentId,
          installDir,
        )} persona and project standards.\n\n`;
        mdContent += "## Role Definition\n\n";
        mdContent += "When the user types `@" + agentId +
        ...
        mdContent += "## Usage\n\n";
        mdContent += `Type \`@${agentId}\` to activate this ${await this
          .getAgentTitle(agentId, installDir)} persona.\n`;
  ...
  async setupGeminiCli(installDir) {
    ...
    if (await fileManager.pathExists(settingsPath)) {
      try {
        ...
        if (
          settings.contextFileName && Array.isArray(settings.contextFileName)
        ) {
