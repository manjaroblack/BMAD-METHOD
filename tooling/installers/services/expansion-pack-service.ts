...
import { join, expandGlob } from "deps";
import type { 
  ExpansionPack,
  ExpansionPackInfo,
...
export class ExpansionPackService implements IExpansionPackService {
  ...
  async getAvailableExpansionPacks(): Promise<ExpansionPack[]> {
    ...
    try {
      ...
      this.logger?.error('Failed to get available expansion packs', error as Error);
      throw new Error(`Failed to retrieve expansion packs: ${(error as Error).message}`);
    }
  }
  ...
  async installExpansionPacks(
    ...
    for (const packId of selectedPacks) {
      try {
        ...
        const result = await this.installSingleExpansionPack(installDir, pack, config);
        if (result.success) {
        ...
        this.logger?.error('Failed to install expansion pack', error as Error, { packId });
      }
    }
  ...
  async detectExpansionPacks(installDir: string): Promise<Record<string, unknown>> {
    ...
    try {
      ...
      if (!await this.fileSystem.exists(packsDir)) {
        this.logger?.debug('Expansion packs directory does not exist', { packsDir });
        return {};
      }
      ...
      this.logger?.error('Failed to detect expansion packs', error as Error, { installDir });
      return {};
    }
  }
  ...
  async updateExpansionPacks(
    ...
    for (const packId of packIds) {
      try {
        ...
        if (!availablePack) {
          this.logger?.warn('Available expansion pack not found for update', { packId });
          continue;
        }

        if (!installedPack) {
          this.logger?.warn('Installed expansion pack not found for update', { packId });
          continue;
        }
        ...
        this.logger?.error('Failed to update expansion pack', error as Error, { packId });
      }
    }
  ...
  async getExpansionPackStatus(
    ...
    try {
      ...
      this.logger?.error('Failed to get expansion pack status', error as Error, { packId });
      return 'corrupted';
    }
  }
  ...
  private async installSingleExpansionPack(
    ...
    try {
      ...
      return {
        ...
        packId: pack.id,
        version: pack.version,
        installedFiles: installedFiles.filter(file => !file.endsWith('/'))
      };
  ...
  private async updateSingleExpansionPack(
    ...
    // Remove old version
    const targetDir = this.fileSystem.join(installDir, this.expansionPacksDir, pack.id);
    if (await this.fileSystem.exists(targetDir)) {
  ...
  private async readExpansionPackInfo(packDir: string): Promise<ExpansionPackInfo | null> {
  ...
  private getExpansionPackSourceDir(packId: string): string {
    ...
    return this.fileSystem.join('extensions', packId);
  }
