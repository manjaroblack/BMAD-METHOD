...
import * as yamlUtils from "../lib/yaml-utils.js";
import { CacheManager, ParallelProcessor, PerformanceMonitor } from "../lib/performance-optimizer.js";
...
class WebBuilder {
  constructor(options = {}) {
    ...
    this.cache = new CacheManager(path.join(this.rootDir, '.bmad-cache', 'web-builder'));
  ...
  async readFileWithCache(filePath) {
    ...
    if (content === null) {
      this.monitor.startTimer(`read_${path.basename(filePath)}`);
      content = await fs.readFile(filePath, 'utf8');
      this.cache.set(cacheKey, content);
      this.monitor.endTimer(`read_${path.basename(filePath)}`);
    }
  ...
  async parseYamlFile(filePath) {
    ...
    if (parsed === null) {
      this.monitor.startTimer(`parse_yaml_${path.basename(filePath)}`);
      const content = await this.readFileWithCache(filePath);
      parsed = this.parseYaml(content);
      this.cache.set(cacheKey, parsed);
      this.monitor.endTimer(`parse_yaml_${path.basename(filePath)}`);
    }
  ...
  async processFilesInParallel(filePaths, processor) {
  ...
  generateWebInstructions(packName = null) {
...
`;
  ...
  async buildAgentBundle(agentId) {
    ...
    sections.push(
      this.formatSection(agentPath, dependencies.agent.content, ".bmad-core"),
    );
    ...
    for (const resource of dependencies.resources) {
      ...
      sections.push(
        this.formatSection(resourcePath, resource.content, ".bmad-core"),
      );
    }
  ...
  async buildTeamBundle(teamId) {
    ...
    sections.push(
      this.formatSection(teamPath, dependencies.team.content, ".bmad-core"),
    );
  ...
  processAgentContent(content) {
    ...
    try {
      ...
      if (
        ...
        parsed["activation-instructions"] = parsed["activation-instructions"]
          .filter(
            (instruction) => {
              return (
    ...
    if (yamlContent) {
      try {
        ...
        if (agentConfig.dependencies) {
          ...
          for (
            const [resourceType, resources] of Object.entries(
  ...
  async buildExpansionTeamBundle(packName, packDir, teamConfigPath) {
    ...
    for (const resourceDir of resourceDirs) {
      ...
      try {
        ...
        for (
          const resourceFile of resourceFiles.filter(
    ...
    if (!agentsToProcess.includes("bmad-orchestrator")) {
      console.warn(
        `    ⚠ Team ${teamFileName} missing bmad-orchestrator, adding automatically`,
      );
